...
hold on
plot(left_points(:,1),left_points(:,2),'g*')
hold on;
plot(el_left(:,1),el_left(:,2),'b*')
hold on;

subplot(122);
imshow(right),title('Points on the right image'),title('Right Image and 15 corresponding points')
hold on
plot(right_points(:,1),right_points(:,2),'g*',el_right(:,1),el_right(:,2),'b*')
hold on;
...
load el_left;
load el_right;
%  el_left = el_left - [1038.*ones(4,1),1042.*ones(4,1)];
%  el_right = el_right - [1038.*ones(4,1),1042.*ones(4,1)];
er_right=el_right;
...
subplot(221);
imshow(left)
title('Points on the left image')
hold on
...
imshow(right);
hold on;
% plot(el_right(:,1),el_right(:,2),'r*');
% hold on;
for i=1:size(al)  
    yl(i,:)=-(al(i)/bl(i)).*xl-(cl(i)/bl(i));
    plot(xl(1,:),yl(i,:),'linewidth', 1.5)
    hold on;  
end
hold on;
title('Corresponding Epipolar lines on right image')
...
imshow(right)
hold on
title('Points on the right image')
plot(er_right(:,1),er_right(:,2),'r*');
hold off
subplot(224);
imshow(left);
title('Corresponding Epipolar lines on left image')
hold on
% plot(el_left(:,1),el_left(:,2),'r*');
for i=1:size(ar)  
    yr(i,:)=-(ar(i)/br(i)).*xr-(cr(i)/br(i));
    plot(xr(1,:),yr(i,:),'linewidth', 1.5)
    hold on;
end 
...
%compute the MSE
MSE=fF_MSE(F,l,r);
s3 = ['Mean squared Sampson error using auto corresponding method is',num2str(MSE),'.'];
disp(s3);